...
impl SqliteGuestTimelineEventRepository {
    pub async fn save(
        ...
        .bind(event.event_type.to_snake())
        .bind(event.reference_id.to_string())
    ...
    fn row_to_event(row: &sqlx::sqlite::SqliteRow) -> AppResult<GuestTimelineEvent> {
        let event_type = TimelineEventType::from_snake(row.get::<String, _>("event_type").as_str())
            .ok_or_else(|| infra("invalid timeline event"))?;
        ...
        Ok(GuestTimelineEvent {
            ...
            reference_id: Uuid::parse_str(row.get::<String, _>("reference_id").as_str())
                .map_err(infra)?,
