...
use crate::{
    ...
    db::connection::Db,
    domain::{
        entity::{deposit_application::DepositApplication, receivable::ReceivableStatus},
        semantic::{
...
pub async fn execute(
    ...
    let result = async {
        if input.amount <= Decimal::ZERO {
            return Err(validation("deposit application amount must be positive"));
        }
        ...
        deposit.apply(input.amount).map_err(conflict)?;

        let application = DepositApplication::new(
            Uuid::new_v4(),
            deposit.id,
            receivable.id,
            input.amount,
