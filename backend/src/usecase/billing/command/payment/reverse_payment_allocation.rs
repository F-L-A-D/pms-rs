...
pub async fn execute(db: &Db, allocation_id: Uuid, reason: Option<String>) -> AppResult<()> {
    ...
    let result = async {
        ...
        let mut payment = SqlitePaymentRepository::find_by_id(&mut tx, allocation.payment_id)
            .await?
            .ok_or_else(|| not_found("payment not found"))?;

        let before_reversed_at = allocation.reversed_at;
        ...
        let new_outstanding_amount = receivable.outstanding_amount + allocation.amount;
        ...
        if before_reversed_at != allocation.reversed_at {
            changed_fields.push(ChangedField::new(
                ...
                allocation.reversed_at.map(|value| value.to_rfc3339()),
            ));
        }
        ...
        if before_payment_unapplied_amount != payment.unapplied_amount {
            changed_fields.push(ChangedField::new(
        ...
        if before_payment_status != payment.status {
            changed_fields.push(ChangedField::new(
                "payment_status",
        ...
        let operation_event = OperationChangeEvent {
            ...
            operation_type: OperationType::ReversePaymentAllocation,
            actor: context.actor,
