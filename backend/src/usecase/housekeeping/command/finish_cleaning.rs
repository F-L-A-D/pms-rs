...
pub async fn execute(db: &Db, input: HousekeepingRoomDailyStateInput) -> AppResult<RoomDailyState> {
    ...
    let result = async {
        ...
        .await?
        .ok_or_else(|| domain("room daily state is not cleaning"))?;

        if state.housekeeping_status != RoomDailyHousekeepingStatus::Cleaning {
            return Err(domain("room daily state is not cleaning"));
        }
        ...
        record_audit_log(
            ...
            RecordAuditLogInput {
                ...
                action: "housekeeping.finish_cleaning".to_string(),
                before_json: None,
                ...
                changed_fields_json: serde_json::json!([
                    {"field_name": "housekeeping_status", "before_value": "cleaning", "after_value": state.housekeeping_status.to_snake()}
                ])
                .to_string(),
                reason: None,
            },
