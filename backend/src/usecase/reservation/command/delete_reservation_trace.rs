...
use crate::{
    api::dto::input::reservation::DeleteReservationTraceInput,
    db::connection::Db,
...
pub async fn execute(
    db: &Db,
    input: DeleteReservationTraceInput,
    ...
    let result = async {
        ...
        SqliteReservationTraceRepository::mark_deleted(
            &mut tx,
            trace.id,
            deleted_at,
            actor_id.as_deref(),
        ...
        record_audit_log(
            ...
            RecordAuditLogInput {
                ...
                action: "reservation.trace.delete".to_string(),
...
fn trace_json(
    trace: &crate::domain::semantic::reservation_trace::ReservationTrace,
) -> serde_json::Value {
    serde_json::json!({
        "id": trace.id,
