...
pub async fn execute(db: &Db, input: RoomDailyStateCommandInput) -> AppResult<RoomDailyState> {
    ...
    let result = async {
        ...
        if !room.is_physical {
            return Err(conflict("virtual room cannot be marked out of order"));
        }

        if !room.is_active {
            return Err(conflict("inactive room cannot be marked out of order"));
        }
        ...
        if state.occupancy_status == RoomDailyOccupancyStatus::Occupied {
            return Err(conflict("occupied room cannot be marked out of order"));
        }

        state.mark_out_of_order();
        ...
        record_audit_log(
            ...
            RecordAuditLogInput {
                ...
                action: "room.out_of_order".to_string(),
                before_json: None,
                ...
                changed_fields_json: serde_json::json!([
                    {"field_name": "occupancy_status", "before_value": null, "after_value": state.occupancy_status.to_snake()}
                ])
                .to_string(),
                reason: None,
            },
