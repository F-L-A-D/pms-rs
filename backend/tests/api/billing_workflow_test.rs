...
use pms_rs::{
    ...
    domain::entity::{deposit::DepositStatus, folio::FolioStatus, payment::PaymentMethod},
...
async fn should_receive_deposit_for_auto_created_reservation_folio() {
    ...
    let deposit: DepositResponse = serde_json::from_value(response_json(response).await).unwrap();

    assert_eq!(deposit.folio_id, folio.id);
    assert_eq!(deposit.amount, Decimal::new(5000, 2));
    assert_eq!(deposit.unapplied_amount, Decimal::new(5000, 2),);
    assert_eq!(deposit.refunded_amount, Decimal::ZERO,);
    assert_eq!(deposit.status, DepositStatus::Held);
    assert_eq!(deposit.method, PaymentMethod::CreditCard);
    assert_eq!(deposit.external_reference.as_deref(), Some("card-auth-001"),);

    let folio_logs = get(&app.app, &format!("/folios/{}/audit", folio.id)).await;
    ...
    assert!(folio_logs
        ...
        .unwrap()
        .iter()
        .any(|log| { log["action"] == "deposit.receive" }));
}
