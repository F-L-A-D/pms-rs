...
use crate::common::{
    ...
    reservation::{
        create_reservation,
        create_reservation_with_external_id,
...
async fn should_filter_reservations_by_external_id() {
    ...
    let response =
        get(
            &app.app,
            "/reservations?external_id=booking-search-001",
        )
        .await;
...
async fn should_return_linked_resources_in_reservation_search_items() {
    ...
    assert_eq!(
        item.linked_resources.primary_guest_id,
        Some(guest.id),
    );

    assert_eq!(
        item.linked_resources.assigned_room_id,
        Some(room.id),
    );
}