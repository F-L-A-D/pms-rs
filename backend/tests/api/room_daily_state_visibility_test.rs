...
use crate::common::{
    ...
    reservation::{cancel_reservation, create_reservation, mark_no_show},
    room::create_room,
    stay::assign_room,
};
...
async fn should_list_rooms_with_daily_state_after_room_is_marked_out_of_order() {
    ...
    let response = get(&app.app, &format!("/rooms?service_date={}", service_date)).await;
    ...
    assert_eq!(room_json["daily_state"]["room_id"], room.id.to_string(),);
    assert_eq!(room_json["daily_state"]["service_date"], service_date,);
    assert_eq!(room_json["daily_state"]["occupancy_status"], "out_of_order",);
}
...
async fn should_list_rooms_with_daily_state_after_housekeeping_transition() {
    ...
    assert_eq!(room_json["daily_state"]["occupancy_status"], "vacant",);
    assert_eq!(room_json["daily_state"]["housekeeping_status"], "cleaning",);
}
...
async fn should_get_room_with_daily_state_for_service_date() {
    ...
    assert_eq!(body_json["daily_state"]["room_id"], room.id.to_string(),);
    assert_eq!(body_json["daily_state"]["service_date"], service_date,);
    assert_eq!(body_json["daily_state"]["housekeeping_status"], "dirty",);
}
...
async fn should_show_assigned_room_as_assigned_but_not_occupied_before_check_in() {
    ...
    assert_eq!(room_json["assignment"]["assignment_status"], "assigned",);
    assert_eq!(
        room_json["assignment"]["reservation_id"],
        reservation.id.to_string(),
    );
    assert_eq!(room_json["assignment"]["stay_status"], "confirmed",);
...
async fn should_show_no_show_room_assignment_as_warning() {
    ...
    assert_eq!(room_json["assignment"]["assignment_status"], "unassigned",);
    assert_eq!(
        room_json["assignment"]["reservation_id"],
        reservation.id.to_string(),
    );
