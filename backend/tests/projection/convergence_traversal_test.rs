use pms_rs::projection::{
    ...
    topology::{
        invalidation_traversal_planner::derive_convergence_plan, projection_node::ProjectionNode,
    },
};
...
fn should_derive_deterministic_convergence_order() {
    ...
    let first = derive_convergence_plan(&invalidation);

    let second = derive_convergence_plan(&invalidation);

    assert_eq!(first.convergence_nodes(), second.convergence_nodes(),);
