...
impl SqliteFolioEntryRepository {
    pub async fn save(tx: &mut Transaction<'_, Sqlite>, entry: &FolioEntry) -> AppResult<()> {
        sqlx::query(
            r#"
        ...
        .bind(entry.entry_type.to_snake())
    ...
    fn row_to_entry(row: &sqlx::sqlite::SqliteRow) -> AppResult<FolioEntry> {
        let entry_type = FolioEntryType::from_snake(row.get::<String, _>("entry_type").as_str())
            .ok_or_else(|| infra("invalid entry type"))?;
