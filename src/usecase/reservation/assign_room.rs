...
pub async fn assign_room(db: &Db, reservation_id: Uuid, room_id: &str) -> AppResult<()> {
    ...
    let result = async {
        ...
        if res.room_id.is_some() {
            return Err(AppError::Conflict("already assigned".into()));
        }

        if room.occupancy_status != OccupancyStatus::Vacant {
            return Err(AppError::Conflict("room not vacant".into()));
        }
